...
- 许可记录方案编码与窗口编码；提交或放行时后端在同一事务内重读两者，只有方案已批准、窗口为 `safe` 且窗口版本与许可预期一致才继续双人确认，许可状态与审计整体成功或整体失败。
- 窗口或方案随后变更时，待审许可在许可页展示失效原因并禁止放行，已放行历史不受影响；联锁依据由后端实时计算并随列表/详情返回，刷新后可回读。
