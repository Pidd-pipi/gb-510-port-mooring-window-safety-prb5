...
- 许可—方案—窗口联锁：许可记录 `planCode`/`windowCode` 及两者的依据版本；提交或放行在单个数据库事务内重读方案与窗口，只有方案为 `approved`、窗口为 `safe` 且窗口（及方案）版本与许可预期一致时才继续双人确认；许可状态更新与审计写入同生共死。
- 方案或窗口随后变更时，待审许可在列表/详情中回读展示失效原因（状态不符、版本漂移或记录缺失）并禁止放行；原提交人可基于新依据重新提交，已放行历史不受影响且不可重新绑定依据。
- `ClearancePanel` 在风浪窗口和许可页共用，许可页另含联锁依据明细表并定时回读；放行审计显式保存窗口版本、方案/窗口编码与版本、操作者和请求 ID。
